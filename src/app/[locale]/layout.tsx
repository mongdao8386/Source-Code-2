...
import { routing, safeLocale, type Locale } from '@/i18n/routing';
...
export async function generateMetadata({
  ...
  // The title suffix and description used to be one fixed string — Vietnamese
  // on the English site, English on the Vietnamese one.
  const [settings, t] = await Promise.all([
    getSiteSettings(),
    getTranslations({ locale: safeLocale(locale), namespace: 'meta' }),
  ]);
